...
// Single source of truth — passed to every row so header & data always align
export const TABLE_GRID =
  "grid grid-cols-[minmax(0,1.4fr)_minmax(0,1.2fr)_110px_120px_100px] items-center gap-x-4 px-4";

// Fixed: dark is zinc-500 not zinc-600 — legible on zinc-900 bg
const COL_HEADER =
...
export function SubmissionsTable({ submissions }: { submissions: RecentSubmission[] }) {
  return (
    <div className="overflow-hidden rounded-xl border border-zinc-100 bg-white dark:border-zinc-800 dark:bg-zinc-900">
      ...
      {/* Header */}
      <div className={`${TABLE_GRID} border-b border-zinc-100 py-2.5 dark:border-zinc-800`}>
